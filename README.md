...
| | [plex-dance.py](plex-dance.py) | Use the output from plex-find-broken-albums.py to automate [Plex Dancing](https://www.plexopedia.com/plex-media-server/general/plex-dance/) the broken albums. Automating Plex Dancing should only be considered when there are a large number of albums to Dance, and only after having made a library backup or filesystem snapshot that can be used to roll-back if requied.<BR><BR>**WARNING:** This script temporarily moves broken albums out of your library to a temporary directory above your library on the same filesystem. Once it has confirmed Plex no longer sees the album (Plex Dancing), it moves the albums back so Plex can rescan them. **Read all the script's instructions carefully. Do not interrupt it while running.** After successfully Plex Dancing, the album will be removed from playlists and any metadata such as ratings will need to be re-applied. |
